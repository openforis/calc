...
-- fact table
drop table if exists naforma1._mgmt_proposal_occurrence cascade;
    
create table naforma1._mgmt_proposal_occurrence as 
SELECT distinct
    p.country_id,
    p.zone_id,
    p.region_id,
    ...
    coalesce(h.code, '-1') as mgmt_proposal,    
    111 as est_area,
    1 as cnt,
    1 as agg_cnt
FROM
...
--- Country
drop table if exists naforma1._country_mgmt_proposal_agg cascade;
    
create table naforma1._country_mgmt_proposal_agg as 
SELECT
    p.country_id,
    p.stratum,
    p.vegetation_type,
