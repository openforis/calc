...
-- fact table
drop table if exists naforma1._biodiversity_occurrence cascade;
    
create table naforma1._biodiversity_occurrence as 
SELECT distinct
    p.country_id,
    p.zone_id,
    p.region_id,
    ...
    coalesce(h.type, '-1') as biodiversity,    
    111 as est_area,
    1 as cnt,
    1 as agg_cnt
FROM
...
--- Country
drop table if exists naforma1._country_biodiversity_agg cascade;
    
create table naforma1._country_biodiversity_agg as 
SELECT
    p.country_id,
    p.stratum,
    p.vegetation_type,
