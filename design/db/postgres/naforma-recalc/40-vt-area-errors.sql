...
alter table _plot
add column primary_vegetation_type varchar(255);

update _plot
set primary_vegetation_type = v.p_code
from mino.vegetation_type  v
where _plot.vegetation_type = v.code;

alter table _country_plot_agg
add column primary_vegetation_type varchar(255);

update _country_plot_agg
set primary_vegetation_type = v.p_code
from mino.vegetation_type  v
where _country_plot_agg.vegetation_type = v.code;

alter table _country_stratum_plot_agg
add column primary_vegetation_type varchar(255);

update _country_stratum_plot_agg
set primary_vegetation_type = v.p_code
from mino.vegetation_type  v
where _country_stratum_plot_agg.vegetation_type = v.code;

// NOTE: In the below, "plots" always implies planned, accessible plot centers

// No. clusters per stratum (ncluster2) (>=0)
...
where
    ...
    p.measurement = 'P' and 
    p.subplot = 'A' and
    p.primary_vegetation_type is not null
group by
    p.cluster_id,
    p.stratum,
    p.primary_vegetation_type;
