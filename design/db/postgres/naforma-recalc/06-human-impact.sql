...
-- fact table
drop table if exists naforma1._human_impact_occurrence cascade;
    
create table naforma1._human_impact_occurrence as 
SELECT distinct
    p.country_id,
    p.zone_id,
    p.region_id,
    ...
    coalesce(ht.code, '-1') as human_impact,
    111 as est_area,
    1 as cnt,
    1 as agg_cnt
FROM
...
--- Country
drop table if exists naforma1._country_human_impact_agg cascade;
    
create table naforma1._country_human_impact_agg as 
SELECT
    p.country_id,
    p.stratum,
    p.vegetation_type,
