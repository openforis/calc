	...
	protected void execute() throws Throwable {
		...
		for ( FactTable factTable : factTables ) {
			for ( AoiHierarchy aoiHierarchy : aoiHierarchies ) {
				...
				List<AggregateTable> aggTables = factTable.getAggregateTables(aoiHierarchy);
				...
				for ( AggregateTable aggTable : aggTables ) {
					...
					if ( isDebugMode() ) {
						psql().dropTableIfExists(aggTable).execute();
					}

					select.addJoin(expf, stratumId.eq(expf.STRATUM_ID).and(aoiId.eq(expf.AOI_ID)).and(expf.ENTITY_ID.eq(entityId)));

					psql().createTable(aggTable).as(select).execute();

					// Grant access to system user
					psql().grant(Privilege.ALL).on(aggTable).to(getSystemUser()).execute();
				}
			}
