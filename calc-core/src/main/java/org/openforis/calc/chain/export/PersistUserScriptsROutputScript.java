...
import org.openforis.calc.r.CalcGetQuotedFileContent;
import org.openforis.calc.r.DbQuoteString;
import org.openforis.calc.r.FileInfo;
import org.openforis.calc.r.Paste;
import org.openforis.calc.r.RScript;
import org.openforis.calc.r.RVariable;
import org.openforis.calc.r.RVector;
import org.openforis.calc.r.ReadChar;
...
public class PersistUserScriptsROutputScript extends ROutputScript {
	...
	private static final String FILE_CONTENT_QUOTED	= "fileContentQuoted";
	...
	private static RScript createScript(Schemas schemas, CommonROutputScript commonScript, BaseUnitWeightROutputScript weightScript,
		...
		addSetFileContentScript( r, commonScript );

		if( weightScript != null ){
			addSetFileContentScript( r, weightScript );
		}
		
		if( CollectionUtils.isNotEmpty(plotAreaScripts) ){
			for (EntityPlotAreaROutputScript plotArea : plotAreaScripts) {
				addSetFileContentScript( r, plotArea );
			}
		}
		
		if( CollectionUtils.isNotEmpty(chainScripts) ){
			for (CalculationStepROutputScript stepScript : chainScripts) {
				addSetFileContentScript( r , stepScript );
			}
		}
		
		if( errorFunctionsScript != null ){
			addSetFileContentScript( r, errorFunctionsScript );
		}
		
//		RVariable filePath 		= r().variable( FILE_PATH+script.getIndex() );
//		Paste paste 			= r().paste( USER_SCRIPT_DIR, r().variable("'"+script.getFileName()+"'") , RScript.PLATFORM$FILE_SEP );
//		SetValue setFilePath	= r().setValue( filePath, paste );
//		r.addScript( setFilePath );
//		
//		RVariable fileContent 	= r().variable( FILE_CONTENT+script.getIndex() );
//		FileInfo fileInfo 		= r().fileInfo( filePath );
//		RVariable size 			= r().variable( fileInfo , "size" );
//		ReadChar readChar 		= r().readChar( filePath , size );
//		SetValue setFileContent = r().setValue( fileContent , readChar );
//		r.addScript( setFileContent );
//		
//		RVariable fileContentQuoted = r().variable( FILE_CONTENT_QUOTED + script.getIndex() );
//		DbQuoteString quoteString 	= r().dbQuoteString( CONNECTION_VAR, fileContent );
//		SetValue setFileContentQ	= r().setValue( fileContentQuoted , quoteString );
//		r.addScript( setFileContentQ );
//		
...
//		}
	...
	private static void addSetFileContentScript(RScript r, ROutputScript script) {
		String fileContentName 							= FILE_CONTENT+script.getIndex();
		RVariable fileContent 							= r().variable( fileContentName );
		CalcGetQuotedFileContent getQuotedFileContent	= r().calcGetQuotedFileContent( r().variable("'"+script.getFileName()+"'") );
		SetValue setFileContent 						= r().setValue( fileContent , getQuotedFileContent );
		r.addScript( setFileContent );
//		r.addScript( r().rScript("print("+fileContentName+")") );
	}
