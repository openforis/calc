 ...
 *
 */
public class CollectDatabaseExporter implements DatabaseExporter {

	private DSLContext context;
	
	public CollectDatabaseExporter(Configuration configuration) {
		super();
		this.context = new DefaultDSLContext(configuration);
	}
	...
	private void insertDataset(RelationalSchema schema, Dataset dataset) throws CollectRdbException {
		...
		try {
			for (int rowno = 0; rowno < rows.size(); rowno++) {
				...
				InsertQuery<Record> insert = context.insertQuery(DSL.tableByName(schema.getName(), table.getName()));
